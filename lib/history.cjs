...
function walk(root, predicate) {
  ...
  while (stack.length) {
    ...
    for (const entry of fs.readdirSync(current, { withFileTypes: true })) {
      ...
      if (entry.isDirectory() && entry.name !== "_camera_prefit") stack.push(full); else if (!entry.isDirectory() && (!predicate || predicate(full))) files.push(full);
    }
  }
  return files.sort();
}
...
function renderRecord(file, rendersRoot, cameras) {
  ...
  const issues = [];
  if (image.width && image.height && !allowed.some(([width, height]) => image.width === width && image.height === height)) issues.push("Unexpected size");
  ...
  return walk(jobsRoot, file => file.toLowerCase().endsWith(".job.json")).map(jobPath => {
    try {
      ...
      const models = (job.tasks || []).map(task => {
        ...
        const files = within(outputFolder, rendersRoot) ? walk(outputFolder, file => imagePattern.test(file)) : [];
      ...
      const timestamps = models.flatMap(model => model.renders.map(render => {
        const file = path.resolve(rendersRoot, decodeURIComponent(render.url.split("path=")[1] || ""));
      ...
      return {
        ...
        modelCount: models.length, renderCount, expectedRenders: expected, state, lastRunState: isCurrent ? currentRender.state : null, models
      };
    } catch (error) {
