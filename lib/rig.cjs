...
// Every product type the pipeline can render, with the cameras it is shot from. A sofa is
// shot from one angle more than a sectional, and from different ones.
const PRODUCT_CAMERAS = { Sectionals: ["F", "FH", "TQ"], Sofas: ["F", "P", "TQ", "TQB"] };
const CAMERAS = [...new Set(Object.values(PRODUCT_CAMERAS).flat())];

// Sectional rows name their scene in the sheet because each side has its own; a sofa has no
// sides, so its rows leave that column empty and the scene follows from what it is and where
// it stands. Both end up as the prefix the sublevels and sequences are named after.
function sceneKeys(row) {
  const named = splitList(row.sequence_prefix);
  if (named.length) return named;
  const stem = String(row.airtable_categories || "").trim().replace(/s$/i, "");
  const place = String(row.environment || "").trim();
  return stem && place ? [`${stem}_${place}`] : [];
}

function filterRows(rows) {
  return rows.filter(row => isActive(row.active) && PRODUCT_CAMERAS[row.airtable_categories]
    && row.environment === "Indoor"
    && PRODUCT_CAMERAS[row.airtable_categories].some(camera => splitList(row.camera).includes(camera)));
}
...
function buildRig(rows) {
  ...
  for (const row of filterRows(rows)) {
    ...
    const allowed = PRODUCT_CAMERAS[row.airtable_categories] || CAMERAS;
    for (const scene of sceneKeys(row)) for (const camera of splitList(row.camera)) {
      if (!allowed.includes(camera)) continue;
...
// A sectional has a scene per side but one set of sublevels, so the side falls away:
// Sectional_Indoor_R lights live in Sectional_Indoor_Background. A sofa has no side, so its
// scene is already the prefix.
const sublevelPrefix = scene => String(scene || "").replace(/_(R|L|U)$/, "");
...
function jobLights(rig, scene, camera, variant = "Default") {
  ...
  return Object.keys(base).sort().map(name => {
    ...
    return {
      ...
      LevelName: `${sublevelPrefix(scene)}_${light.sublevel}`
    };
  });
}
...
module.exports = { SheetStore, buildRig, filterRows, jobLights, sceneKeys, sublevelPrefix, PRODUCT_CAMERAS, CAMERAS, LIGHTS_URL };
