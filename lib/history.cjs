...
function walk(root, predicate) {
  ...
  while (stack.length) {
    ...
    for (const entry of fs.readdirSync(current, { withFileTypes: true })) {
      ...
      if (entry.isDirectory() && !["_camera_prefit", "_crop_calibration", READY_FOLDER_NAME, LEGACY_READY_FOLDER_NAME].includes(entry.name)) stack.push(full); else if (!entry.isDirectory() && (!predicate || predicate(full))) files.push(full);
    }
  }
  return files.sort();
}
...
function renderRecord(file, rendersRoot, cameras, deliveryFile = null) {
    ...
    try {
      ...
      const models = (job.tasks || []).map(task => {
        ...
        return {
          ...
          outputFolder, expectedRenders: expectedRenders(task), renders: files.map(file => renderRecord(file, rendersRoot, cameras, processedBySource.get(path.resolve(file).toLowerCase()))),
