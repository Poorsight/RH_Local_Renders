...
function calibrationLayer(layerName, outputFolder, sourceLayer) {
  const sublevels = sourceLayer?.SubLevels?.length
    ? [...sourceLayer.SubLevels]
    : layerName === "Fabric"
      ? ["Sectional_Indoor_Background", "Sectional_Indoor_KeyLight"]
      : ["Sectional_Indoor_Shadow", "Sectional_Indoor_KeyLight"];
...
function calibrationJob(job, layerName, index, count) {
  ...
  result.tasks = (result.tasks || []).map(task => {
    ...
    return { ...task, sequence: { ...(task.sequence || {}), cameras: pendingCameras }, layers: [calibrationLayer(layerName, outputFolder, (task.layers || []).find(layer => layer.name === layerName))] };
  }).filter(Boolean);
