...
- **Render previews**: server-side images under `Renders/` next to `index.html`. Manifest-first: `Renders/manifest.json` (generated by `scripts/gen-render-manifest.cjs`) drives the board and discovers material folders; without it the board falls back to per-file probing of the hardcoded material list. No drag-drop and no IndexedDB image storage. Comments are **shared** through `comments.php` (one note per image, stored server-side); per-browser `localStorage` is the fallback when the endpoint is unavailable.
