...
function fitDescriptor(task, camera, options = {}) {
  ...
  const inputs = {
    ...
    // The frame is part of what a fit means: a focal length measured against one sensor and
    // pixel count does not frame another the same way.
    frame: (camera?.LayerResolutions || []).map(layer => ({
      name: layer?.Name || "", resolution: layer?.Resolution || null, sensor: layer?.SensorSize || null
    })),
