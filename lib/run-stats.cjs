...
const MAX_RUNS = 200;
// Redirectable, because the tests drive the real server against the real tree: their
// stubbed renders finish in milliseconds, and folded into the averages they would price
// an hours-long job in seconds.
const statsFile = root => process.env.RH_RUN_STATS_FILE || path.join(root, "local", "cache", "run-stats.json");
  ...
  for (const task of job?.tasks || []) {
    ...
    for (const layer of task.layers || []) {
      ...
      const variants = String(layer.name || "").toLowerCase() === "fabric"
        ...
        : 1;
      counts[layer.name] = (counts[layer.name] || 0) + cameras * variants;
    }
  }
  return counts;
}
