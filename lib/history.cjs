...
const { READY_FOLDER_NAME, LEGACY_READY_FOLDER_NAME, isProcessedImage, materialFor, processedPathFor, selectedMaterialsFor } = require("./post-process.cjs");
const { materialCombinationCount } = require("./jobs.cjs");
...
function renderRecord(file, rendersRoot, cameras, deliveryFile = null, declared = null, task = null) {
  ...
  const selectedMaterials = layer === "Fabric" ? selectedMaterialsFor(file, task) : [];
  return {
    name, camera, layer, size: fs.statSync(file).size, ...image, issues,
    material: layer === "Fabric" ? materialFor(file, task) : "",
    materials: selectedMaterials.map(group => group.name).filter(Boolean),
...
function history(root, currentRender = {}) {
  ...
  return walk(jobsRoot, file => file.toLowerCase().endsWith(".job.json")).map(jobPath => {
    try {
      ...
      // Current deliveries keep manifest.json beside POST so the delivery folder contains only
      // uploadable model folders. Older builds placed it inside POST or _READY_TO_UPLOAD.
      const besideReadyManifest = path.join(outputFolder, "manifest.json"), inReadyManifest = path.join(readyFolder, "manifest.json"), legacyManifest = path.join(legacyReadyFolder, "manifest.json");
      const [resolvedReadyFolder, readyManifest] = fs.existsSync(besideReadyManifest) && fs.existsSync(readyFolder)
        ? [readyFolder, besideReadyManifest]
        : fs.existsSync(inReadyManifest) ? [readyFolder, inReadyManifest] : [legacyReadyFolder, legacyManifest];
      ...
      const models = (job.tasks || []).map(task => {
        ...
        return {
          ...
          outputFolder, expectedRenders: expectedRenders(task), renders: files.map(file => renderRecord(file, rendersRoot, cameras, processedBySource.get(path.resolve(file).toLowerCase()), declared, task)),
      ...
      return {
        ...
        renderEnvironment: job._rhLocal?.renderEnvironment || (job._rhLocal?.models || [])[0]?.renderEnvironment || "ue56",
