...
function buildConstants({ L, BUILTIN }) {
  ...
  return {
    ...
    rig_scale: {
      rule: "the three axis ratios collapse into ONE factor: k = max(1, cbrt(sX * sY * sZ)). Every light gets the same k in X/Y; Z is always copied from the source rig. Pitch/yaw/roll are never recomputed.",
    ...
    validation: {
      ...
      shape_mismatch: "warn when max(sX,sY,sZ)/min(sX,sY,sZ) > 1.5 - one X/Y footprint factor follows the sofa's size but not its shape",
