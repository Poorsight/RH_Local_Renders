...
// Material assignment groups use the final token of an Unreal mesh name. Keep the
// checker on that same rule: exporters may prefix the actual part with an entire
// product/mesh path, while trailing numbers are only duplicate-name artefacts.
const partKey = value => {
  const source = String(value || "").trim().toLowerCase();
  const last = source.split(/[:_]/).filter(Boolean).pop() || source;
...
function checkModel(model, record) {
  ...
  // 4. Parts, reduced exactly as the assignment UI reduces them: the final colon/underscore
  // token is the semantic part and digits are export artefacts, not a different part.
