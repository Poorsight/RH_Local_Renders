...
// The inspector measures how many metres a raw unit is worth, and that same factor is what
// the job hands to Unreal: a tracked inch model carries 2.54 and renders correctly with it,
// which is also the value the farm sends. So the factor passes through untouched — what a
// check can usefully say is whether the size it produces is one a piece of furniture has.
...
// One place decides the factor a job carries, so a model of any scale is handled by
// measuring it rather than by assuming a format's units.
function unrealScaleFor(record) {
...
function checkModel(model, record) {
  ...
  const scale = unrealScaleFor(record);
  if (scale === null) {
    add("error", "no-scale", "No scale", "The inspector produced no unit factor, so the model would import at raw size.");
  } else {
    add("ok", "scale", "Scale", `${record.analysis?.unit || "units"} measured; job sends offsetUniformScale ${scale}.`);
  }
...
module.exports = { SIZE_LIMITS, REQUIRED_PARTS, partKey, productType, unrealScaleFor, checkModel, summarise };
