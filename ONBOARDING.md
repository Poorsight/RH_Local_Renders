...
- **Render previews**: server-side images under `Renders/` next to `index.html`. Manifest-first: `Renders/manifest.json` (generated by `scripts/gen-render-manifest.cjs`) drives the board and discovers material folders; without it the board falls back to per-file probing of the hardcoded material list. No drag-drop and no IndexedDB image storage. Comments are local, keyed by image URL in `localStorage`.
