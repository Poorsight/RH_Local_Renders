...
(async () => {
  ...
  for (const task of tasks) {
    ...
    for (const [index, camera] of (task.sequence?.cameras || []).entries()) {
      fs.writeFileSync(path.join(output, `00000000_${camera.name}_${phase}.png`), `${phase}:${task.taskId}:${camera.name}:${Date.now()}`);
      if (phase === "Fabric") await fetch(apiUrl, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({
