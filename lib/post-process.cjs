...
function deliveryRecordsFor(job, files) {
  const records = files.map(file => {
    ...
    return { file, task, model: safeDeliverySegment(task.taskId), camera, layer, material: materialFor(file, task) };
  });
  ...
  for (const record of records) {
    ...
    usedNames.add(`${record.model.toLowerCase()}::${name.toLowerCase()}`);
    record.name = name; record.destination = `${record.model}/${name}`;
  }
  return records;
}
...
function commitDelivery(job, outputRoot, staging, records, expected) {
  ...
  const manifest = {
    ...
    files: records.map(record => ({
      model: record.model, camera: record.camera, layer: record.layer, material: record.material,
