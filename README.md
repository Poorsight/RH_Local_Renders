...
At the end of every post-process pass, the service atomically rebuilds `<batch>/_READY_TO_UPLOAD/`. It contains one folder per model and clean upload names such as `<MODEL>_F.png` and `<MODEL>_F_Shadow.png`; jobs with multiple Fabric materials add the material to avoid collisions. `manifest.json` records every source mapping and whether all expected files are present. These are copies of the processed results, so the RAW and `_POST` files used by history and previews remain untouched. The generated delivery folder is excluded from render discovery.
