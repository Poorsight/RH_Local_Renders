...
const { READY_FOLDER_NAME, isProcessedImage, processedPathFor } = require("./post-process.cjs");
...
function walk(root, predicate) {
  ...
  while (stack.length) {
    ...
    for (const entry of fs.readdirSync(current, { withFileTypes: true })) {
      ...
      if (entry.isDirectory() && !["_camera_prefit", "_crop_calibration", READY_FOLDER_NAME].includes(entry.name)) stack.push(full); else if (!entry.isDirectory() && (!predicate || predicate(full))) files.push(full);
    }
  }
  return files.sort();
}
...
function history(root, currentRender = {}) {
  ...
  return walk(jobsRoot, file => file.toLowerCase().endsWith(".job.json")).map(jobPath => {
    try {
      ...
      const readyFolder = path.join(outputFolder, READY_FOLDER_NAME), readyManifest = path.join(readyFolder, "manifest.json");
      let readyToUpload = null;
      if (fs.existsSync(readyManifest)) {
        try {
          const manifest = JSON.parse(fs.readFileSync(readyManifest, "utf8"));
          readyToUpload = { folder: readyFolder, files: Number(manifest.fileCount) || 0, models: Number(manifest.modelCount) || 0, complete: Boolean(manifest.complete) };
        } catch { readyToUpload = { folder: readyFolder, files: 0, models: 0, complete: false }; }
      }
