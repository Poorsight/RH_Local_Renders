...
function taskFor(input, model, rig, outputFolder) {
  ...
  const profileName = renderProfile(input.renderProfile), cropModeName = cropMode(input.cropMode), resolutions = RESOLUTION_PROFILES[profileName];
...
module.exports = { CAMERA_YAW, RESOLUTIONS, RESOLUTION_PROFILES, renderProfile, cropMode, buildJob, buildBatchJob, writeJob, writeBatchJob, groupedMaterials, fabricFileNameFormat };
