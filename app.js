(() => {
  ...
  const renderHistoryDetail = () => {
    ...
    $("historyDetail").innerHTML = `<div class="history-detail-heading"><div><span>SAVED JOB</span><strong>${escapeHtml(batch.id)}</strong><small>${escapeHtml(formatDate(batch.generatedAt))}</small></div><i class="history-state" data-state="${escapeHtml(batch.state)}">${escapeHtml(historyStateLabel(batch.state))}</i></div><div class="history-summary"><div><span>MODELS</span><strong>${batch.modelCount}</strong></div><div><span>RENDERS</span><strong>${batch.renderCount}/${batch.expectedRenders}</strong></div><div><span>POST</span><strong>${batch.postProcessCount || 0}/${batch.renderCount}</strong></div></div>${selective}${models}${batch.error ? `<p class="inline-warning">${escapeHtml(batch.error)}</p>` : ""}<code class="history-path" title="${escapeHtml(batch.jobPath)}">${escapeHtml(batch.jobPath)}</code><div class="history-actions"><button class="primary-button" type="button" data-history-action="selective"${batch.modelCount ? "" : " disabled"}>Load selection</button><button class="secondary-button" type="button" data-history-action="edit"${batch.modelCount ? "" : " disabled"}>Load all & edit</button><button class="secondary-button" type="button" data-history-action="review"${batch.modelCount ? "" : " disabled"}>Review rig</button><button class="secondary-button" type="button" data-history-action="rerun"${batch.state === "invalid" ? " disabled" : ""}>Run again</button><button class="secondary-button" type="button" data-history-action="postprocess"${batch.renderCount ? "" : " disabled"}>Post-process</button><button class="quiet-button" type="button" data-history-action="viewJob">View JSON</button><button class="quiet-button" type="button" data-history-action="showJob">Show JSON</button><button class="quiet-button" type="button" data-history-action="openRenders"${batch.renderCount ? "" : " disabled"}>Open renders</button></div>`;
  };
  ...
  const renderRigGallery = () => {
    ...
    const card = render => {
      const display = render.processed || render, issues = render.processed ? display.issues || [] : render.issues || [];
      const diagnostics = [display.width && display.height ? `${display.width}×${display.height}` : "Unknown size", display.alpha === true ? "Alpha" : display.alpha === false ? "No alpha" : "Alpha unknown", ...(issues || [])];
      return `<a class="render-preview-card${issues.length ? " render-warning" : ""}" data-layer="${escapeHtml(render.layer || "Fabric")}" href="${escapeHtml(display.url)}" target="_blank" rel="noreferrer"><div class="render-preview-media" style="--preview-aspect:${Number(display.width) || 1}/${Number(display.height) || 1}"><img src="${escapeHtml(display.url)}" alt="${escapeHtml(model.name)} ${escapeHtml(render.camera || "render")} ${escapeHtml(render.layer || "")}" loading="lazy"></div><span>${escapeHtml(render.layer || render.camera || render.name)} · ${render.processed ? "POST" : "RAW"}${issues.length ? " · Check" : ""}</span><small>${escapeHtml(diagnostics.join(" · "))}</small></a>`;
    };
    const combinedCard = (fabric, shadow) => {
      if (!fabric || !shadow) return "";
      const fabricDisplay = fabric.processed || fabric, shadowDisplay = shadow.processed || shadow;
      const fabricWidth = Math.min(100, Math.max(1, (Number(fabricDisplay.width) || 1) / (Number(shadowDisplay.width) || 1) * 100));
      const issues = [...(fabricDisplay.issues || []), ...(shadowDisplay.issues || [])], isPost = Boolean(fabric.processed && shadow.processed);
      return `<button type="button" class="render-preview-card render-combined${issues.length ? " render-warning" : ""}" data-layer="Combined" data-fabric-url="${escapeHtml(fabricDisplay.url)}" data-shadow-url="${escapeHtml(shadowDisplay.url)}" data-fabric-width="${fabricWidth}" data-combined-title="${escapeHtml(`${model.name} · ${fabric.camera || "render"} · Combined`)}"><div class="render-preview-media" style="--preview-aspect:${Number(shadowDisplay.width) || 1}/${Number(shadowDisplay.height) || 1};--fabric-width:${fabricWidth}%"><img class="render-composite-shadow" src="${escapeHtml(shadowDisplay.url)}" alt="" loading="lazy"><img class="render-composite-fabric" src="${escapeHtml(fabricDisplay.url)}" alt="${escapeHtml(model.name)} ${escapeHtml(fabric.camera || "render")} Fabric and Shadow combined" loading="lazy"></div><span>Combined · ${isPost ? "POST" : "RAW"}${issues.length ? " · Check" : ""}</span><small>Fabric over Shadow · click to open</small></button>`;
    };
  ...
  $("historyDetail").addEventListener("click", async event => {
    ...
    try {
      ...
      else if (action === "postprocess") { const result = await api("/api/postprocess", { method: "POST", body: JSON.stringify({ jobPath: batch.jobPath }) }); toast("Post-processing started; originals stay unchanged"); updateRender(result); startPolling(); }
