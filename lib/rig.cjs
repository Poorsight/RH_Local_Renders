...
function jobLights(rig, scene, camera, dimensions, mode = "B", variant = "Default") {
  ...
  return LIGHT_ORDER.map(name => {
    const light = base[name], settings = String(variant).toLowerCase() === "shadow" ? light.shadow : light, geometry = SOURCE_GEOMETRY[name] || {};
    const [x, y, z] = settings.position, d2 = x * x + y * y + z * z, radius2 = (geometry.effectiveRadius || 0) ** 2;
    const intensity = mode === "A" ? settings.intensity * k * k : settings.intensity * ((k * k * d2 + radius2) / (d2 + radius2));
    const sizeScale = mode === "A" ? k : 1;
    const result = {
      name, Location: { X: round6(x * k), Y: round6(y * k), Z: round6(z) }, rotation: { ...settings.rotation },
      intensity: round6(intensity), InnerConeAngle: settings.innerConeAngle, OuterConeAngle: settings.outerConeAngle,
