...
function walk(root, predicate) {
  ...
  while (stack.length) {
    ...
    for (const entry of fs.readdirSync(current, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) stack.push(full); else if (!predicate || predicate(full)) files.push(full);
    }
  }
  return files.sort();
}
...
function expectedRenders(task) {
  ...
  return (task.layers || []).reduce((total, layer) => {
    if (layer.doNotRender) return total;
...
function renderRecord(file, rendersRoot, cameras) {
  ...
  return {
    name, camera, size: fs.statSync(file).size,
