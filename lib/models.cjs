...
class ModelStore {
  ...
  result(modelPath, record, source, newlyAnalyzed = false) {
    ...
    return {
      ...
      dimensions: { width, depth, height }, importYaw: record.yaw, offsetUniformScale: unrealScaleFor(record) ?? record.scale,
  ...
  async inspect(query) {
    ...
    const sourceFingerprint = fingerprint(modelPath), cached = this.localMetadata.models[name];
    ...
    if (!this.pending.has(modelPath)) this.pending.set(modelPath, (async () => {
      ...
      this.localMetadata.models[name] = record; this.saveLocal();
