...
const cropProfileFor = (store, fingerprint, camera, contextToken = null) => {
  const profile = store?.profiles?.[profileKey(fingerprint, camera)] || null;
  return profile && (!contextToken || profile.contextToken === contextToken) ? profile : null;
};

// A crop belongs to the effective frame, not to the exact JSON shape a caller happened to
// send. Saved jobs include a display-only Name field and remote clients may send numbers as
// strings; neither changes the rendered frame, so neither may invalidate a crop profile.
function cropContextResolutions(resolutions) {
  if (!resolutions || typeof resolutions !== "object") return null;
  const normalized = {};
  for (const layer of ["Fabric", "Shadow"]) {
    const source = resolutions[layer];
    ...
    normalized[layer] = {
      Resolution: { X: number(source.Resolution?.X), Y: number(source.Resolution?.Y) },
      SensorSize: { X: number(source.SensorSize?.X), Y: number(source.SensorSize?.Y) }
    };
  }
  return Object.keys(normalized).length ? normalized : null;
}

// Drops saved crops so the next optimized run measures them again. A reused crop is normally
// what you want -- it is why the second run of a batch is quick -- but when a scene or a
// light has moved, a crop measured before that is measuring the wrong thing, and there was no
// way to say so short of editing the file by hand.
function forgetCropProfiles(root, { fingerprints, cameras, all } = {}) {
  const store = readCropProfiles(root);
  const wantedCameras = new Set((cameras || []).map(camera => String(camera).toUpperCase()));
  const wantedPrints = new Set(fingerprints || []);
  const dropped = [];
  for (const [key, profile] of Object.entries(store.profiles || {})) {
    const matches = all
      || (wantedPrints.has(profile?.fingerprint) && (!wantedCameras.size || wantedCameras.has(String(profile?.camera || "").toUpperCase())));
    if (!matches) continue;
    dropped.push({ modelName: profile?.modelName || "", camera: profile?.camera || "", analyzedAt: profile?.analyzedAt || null });
    delete store.profiles[key];
  }
  if (dropped.length) {
    const file = cachePath(root);
    fs.mkdirSync(path.dirname(file), { recursive: true });
    fs.writeFileSync(file, JSON.stringify(store, null, 2) + String.fromCharCode(10), "utf8");
  }
  return dropped;
}
...
function calibrationFiles(folder, camera) {
  ...
  const resolved = path.resolve(String(folder || "")), leaf = path.basename(resolved).toLowerCase(), split = ["materials", "shadows"].includes(leaf);
  const modelFolder = split ? path.dirname(resolved) : resolved;
  const filesIn = candidate => fs.existsSync(candidate) ? fs.readdirSync(candidate).filter(name => name.toLowerCase().endsWith(".png") && token.test(name)).map(name => path.join(candidate, name)) : [];
  const fabricFiles = filesIn(path.join(modelFolder, "materials")), shadowFiles = filesIn(path.join(modelFolder, "shadows"));
  // Pre-split batches keep both probes in one folder. Read them as a fallback so old crop
  // profiles remain recoverable after the writer moves to the layered structure.
  const legacyFiles = split ? [] : filesIn(resolved);
  return {
    fabric: [...fabricFiles, ...legacyFiles].find(file => !/(?:^|_)shadow(?:_|\.)/i.test(path.basename(file))) || null,
    shadow: [...shadowFiles, ...legacyFiles].find(file => /(?:^|_)shadow(?:_|\.)/i.test(path.basename(file))) || null
  };
}

module.exports = {
  CACHE_VERSION, CACHE_FILE, CALIBRATION_FOLDER, DEFAULT_MARGIN, OUTPUT_ALIGNMENT,
  modelFingerprint, profileKey, readCropProfiles, writeCropProfiles, cropProfileFor, cropContextResolutions, forgetCropProfiles,
