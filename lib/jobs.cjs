...
function taskFor(input, model, rig, outputFolder) {
  ...
  const layerBlocks = layers.map(name => name === "Fabric" ? {
    ...
    output: { folder: output, fileNameFormat: fabricFileNameFormat(materials), overWrite: true }, doNotRender: false
  } : {
  ...
  return {
    ...
    metadata: { name: model.name, modelPath: model.path, dimensions, side, importYaw, sourceMode: input.sourceMode || "B", renderProfile: profileName, rigScale: rigScale(dimensions), outputFolder: output }
  };
}
